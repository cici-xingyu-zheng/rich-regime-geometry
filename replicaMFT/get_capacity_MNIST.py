...
grand_parent_dir = '/data/cici/Geometry/new/MNIST/activations'
...
output_parent_dir = '../MNIST_results/output/capacity_measures'
...
for i, parent_dir in enumerate(parent_dirs):
    ...
    for subfolder in sorted(os.listdir(parent_dir)):
        ...
        if os.path.isdir(subfolder_path):
            print(f"{i+1}th folder's", subfolder_path, '\n')
            
            # Analyze the subfolder
            results_df = analyze_folder(subfolder_path, kappa, n_t)
            
            # Save the results to a CSV file
            output_dir = os.path.join(output_parent_dir, dirs[i])
