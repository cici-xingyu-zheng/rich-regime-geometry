...
results = {}
for initialization_scale in initialization_scales:

    print(f"alpha: {initialization_scale}")
    # Create model
    mlp = create_mlp(depth, width, activation_fn, 1)
    ...
    results[initialization_scale] = {'train_loss': [], 'test_loss': [],
    ...
    with tqdm(total=optimization_steps) as pbar:
        for x, labels in islice(cycle(train_loader), optimization_steps):
            if (steps < 30) or (steps < 150 and steps % 10 == 0) or steps % log_freq == 0:
                results[initialization_scale]['train_loss'].append(compute_loss(mlp, train, loss_function, device, N=len(train)))
                results[initialization_scale]['train_accuracies'].append(compute_accuracy(mlp, train, device, N=len(train)))
                results[initialization_scale]['test_loss'].append(compute_loss(mlp, test, loss_function, device, N=len(test)))
                results[initialization_scale]['test_accuracies'].append(compute_accuracy(mlp, test, device, N=len(test)))
                results[initialization_scale]['log_steps'].append(steps)
                with torch.no_grad():
                    total = sum(torch.pow(p, 2).sum() for p in mlp.parameters())
                    results[initialization_scale]['norms'].append(float(np.sqrt(total.item())))
                    last_layer = sum(torch.pow(p, 2).sum() for p in mlp.mlp[-1].parameters())
                    results[initialization_scale]['last_layer_norms'].append(float(np.sqrt(last_layer.item())))
                pbar.set_description("L: {0:1.1e}|{1:1.1e}. A: {2:2.1f}%|{3:2.1f}%".format(
                    results[initialization_scale]['train_loss'][-1],
                    results[initialization_scale]['test_loss'][-1],
                    results[initialization_scale]['train_accuracies'][-1] * 100,
                    results[initialization_scale]['test_accuracies'][-1] * 100))
            ...
            if steps in [1, 10, 100, 500, 1000, 5000, 10000, 20000, 30000, 40000, 50000, 60000, 70000, 80000, 90000, 100000]:
                ...
                save_path = os.path.join('/data/cici/Geometry/new/MNIST/checkpoints', f'scale_{initialization_scale}_{steps}.pth')
...
filename = os.path.join('.', f'acc_loss_log_initial_scale.pkl')
